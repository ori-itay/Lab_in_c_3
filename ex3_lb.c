                      ...
                      struct sockaddr_in* http_server_sockaddr)
{
  ...
  (*server_sockaddr).sin_addr.s_addr = INADDR_ANY;  // not sure who initiate the connection..I think LB sends to the
                                                    // servers first, if so, why he must set a listen
  ...
  while (true) {
    ...
    while (true) {
      ...
      if ((end_sequence_of_http_request_pointer = strstr(http_request_buffer, END_OF_HTTP_REQUEST)) !=
          NULL) {  // means an end to an http request has come
                                     ...
                                     int server_sockets_list[NUM_OF_SERVERS])
{
  ...
  *server_number = ((*server_number) + INCREMENT_1) % NUM_OF_SERVERS;
